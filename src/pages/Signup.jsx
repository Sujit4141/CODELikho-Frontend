...
// Updated schema with terms validation
...
function Signup() {
  ...
  useEffect(() => {
   ...
   const onSubmit = (data) => {
    setFormError(null);
    setIsBackendStarting(true); // Set backend starting state
    dispatch(registerUser(data))
      .unwrap()
      .catch(() => setIsBackendStarting(false)); // Reset if request fails
  };
    ...
    <div className={`min-h-screen ${themeClasses.bg} ${themeClasses.text} transition-colors duration-300 flex flex-col items-center justify-center p-4`}>
      ...
      <div className={`w-full max-w-md ${themeClasses.card} rounded-xl shadow-2xl overflow-hidden border ${themeClasses.accent.replace('text', 'border')}/30`}>
        <div className="p-8">
          <div className="text-center mb-8">
            <h2 className={`text-3xl font-bold ${themeClasses.accent} mb-2`}>Create Your Account</h2>
            <p className={`${themeClasses.text} opacity-80`}>Join our community today</p>
          ...
          {formError && (
            <div className={`mb-6 p-3 rounded-lg flex items-center bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400`}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-4">
              {/* First Name Field */}
              <div>
                <label className={`block ${themeClasses.text} text-sm font-medium mb-2`} htmlFor="firstName">
                  First Name
                </label>
                <input
                  id="firstName"
                  ...
                  className={`w-full px-4 py-3 ${themeClasses.card.replace('border-gray-700', '')} border ${
                    errors.firstName ? 'border-red-500' : themeClasses.card.includes('border') ? 
              ...
              <div className='relative'>
                ...
                <div className="relative">
                  ...
                  <button 
                    type='button' 
                    className={`cursor-pointer absolute right-3 top-1/2 transform -translate-y-1/2 p-1 rounded-full hover:bg-gray-700 focus:outline-none`}
                    onClick={toggleEye}
                  >
                    {isEyeOpen ? (
            ...
            {/* Terms and Conditions Checkbox with Error Handling */}
            <div>
              <div className="flex items-center">
                <input 
                  ...
                  {...register("terms")}
                />
                <label htmlFor="terms" className={`ml-2 text-sm ${themeClasses.text} opacity-80`}>
                  I agree to the <a href="#" className={`${themeClasses.accent} hover:underline`}>Terms and Conditions</a>
                </label>
              </div>
              {errors.terms && (
                <p className={`mt-2 text-red-500 dark:text-red-400 text-sm flex items-center`}>
            ...
            <button 
              type="submit" 
              disabled={loading}
              className={`w-full ${themeClasses.button} text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 ${loading ? 'opacity-70 cursor-not-allowed' : 'hover:scale-[1.02]'} shadow-lg ${themeClasses.accent.replace('text', 'shadow')}/20 hover:${themeClasses.accent.replace('text', 'shadow')}/40`}
            >
              {loading ? (
        ...
        <div className={`${themeClasses.header} p-4 text-center`}>
          <p className={`text-sm ${themeClasses.text} opacity-80`}>
            By signing up, you agree to our Privacy Policy and Cookie Policy
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 text-center">
        <div className="flex justify-center space-x-6">
          <button className={`p-3 ${themeClasses.card.replace('border-gray-700', '')} rounded-full hover:bg-gray-700 transition`}>
